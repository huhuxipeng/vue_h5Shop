        ...
        <div id="wrap">
          ...
          <div class="content">
            <div class="conten">
              ...
              <p class="money" v-if="flashsaleShow">￥{{Number(flashsalePrice).toFixed(2)}}</p>
              <p class="money" v-if="!flashsaleShow&&!goldChangerShow">￥{{Number(detail.price).toFixed(2)}}</p>
              <p class="money" v-if="goldChangerShow">{{detail.exchangeCoin}}金币</p>
            </div>
          ...
          <div class="footers" v-if="flashsaleShow&&!hideButton">
            <p class="buynows" @click="purchase" v-if="!noFlashsale">抢购</p>
            <span class="export" v-if="noFlashsale">已告罄</span>
          </div>
          <div class="footers" v-if="goldChangerShow">
            <p class="buynows" @click="purchase" v-if="!noGoldGoods">立即兑换</p>
            <span class="export" v-if="noGoldGoods">已兑完</span>
          </div>
...
export default {
    ...
    if (this.shopdetai[0].goodsSpecList) {
      ...
      for (let i = 0; i < this.goodsSpecList.length; i++) {
        ...
        for (let j = 0; j < this.goodsSpecList[i].detail.length; j++) {
          this.stand(this.goodsSpecList[i].detail[0], i, 0);
  ...
  methods: {
    formattPrice(matchProductId) {
      let price;
      this.product.forEach(o => {
        if (o.productId == matchProductId) {
          price = o.price;
        }
      });
      return price;
    ...
    stand(els, i, j) {
      ...
      if (this.flashsaleShow) {
        let hasType = false;
        this.product.forEach(o => {
          if (o.specValueIds == els.specValueId) {
            this.productId = o.productId;
            this.flashsalePrice = o.flashsalePrice;
            this.noFlashsale = false;
            hasType = true;
          }
        });
        if (!hasType) {
          //如果没有选中的规格
          this.noFlashsale = true;
        }
      }
      sessionStorage.setItem("SELSPECOBJ", JSON.stringify(this.selSpecObj));
    ...
    purchase() {
      ...
      if (this.buyCount > this.store) {
        ...
        if (this.token) {
          ...
          if (this.flashsaleShow) {
            //如果是抢购商品
            url = "/flashsale/rushBuy";
            params = {
              token: this.token,
              buyCount: this.buyCount,
              flashsaleProductId: this.matchProductId //抢购商品ID
            };
          } else if (this.goldChangerShow) {
            //金币兑换商品
            console.log(this.product);
            let perMember = this.detail.exchangeCoin;

            var totolMemberCoin = this.buyCount * perMember;
            url = "/order/coinExchangeOrderCheckout";
            params = {
              token: this.token,
              productId: this.matchProductId, //抢购商品ID
              buyCount: this.buyCount,
              id:this.$route.query.goldChangerId,
            };
            console.log(this.detail.memberCoin, totolMemberCoin);
            if (this.detail.memberCoin < totolMemberCoin) {
              MessageBox(
                "提示",
                "金币不足，您的金币数量为" + this.detail.memberCoin
              );
              return;
            }
          } else {
            ...
            .then(data => {
              if (data.status == 200) {
                ...
                if (data.data.res_code == 1) {
                  ...
                  let goodsList = data.data.res_data;
                  sessionStorage.goodsList = JSON.stringify(goodsList);

                  if (this.flashsaleShow) {
                    this.$router.push({
                      path: "/shoppingCart/balance",
                    ...
                    this.$router.push({
                      path: "/shoppingCart/balance",
                      query: { isGoldChanger: 1 ,goldChangerId:this.$route.query.goldChangerId}
                    });
                    return;
                  }
